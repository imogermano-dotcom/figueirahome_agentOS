...
- **A qualificação vive em `guards.find_or_create_cliente`, não numa tool nova**: é o ponto único por onde todos os caminhos de escrita de cliente passam, por isso todos os canais ganham a regra sem a repetir. MQL = `tipo_interesse` + `orcamento` + `zona_preferida`; `prazo_compra` fica de fora porque só é recolhido noutro sítio e exigi-lo deixava toda a lead de WhatsApp por qualificar.
- **A lead qualificada pára numa tarefa, não escreve no eGO nem em `contactos`**: mantém a decisão em "Dados e esquema". A ordem que resolveria o órfão é criar no eGO primeiro e deixar o sync espelhar de volta com `ego_link` — mas não há API de escrita do eGO configurada (`websiteapi` é `GET` de imóveis, `admin.egorealestate.com` é backoffice por scraping de sessão). Enquanto não houver chave de integração, a passagem é manual.
...
- **`leads` guarda telefone e email na própria linha**: `leads_angariacao` depende de um join a `contactos` por `(nome, criado_em)` que hoje resolve 74 de 79, mas parte-se assim que dois leads com o mesmo nome cheguem no mesmo dia — e sem telefone não há follow-up nenhum.
