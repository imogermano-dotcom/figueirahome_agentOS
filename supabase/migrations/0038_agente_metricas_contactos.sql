-- ════════════════════════════════════════════════
-- Migration 0038 — agente_metricas: leads_captados via contactos (por agente)
-- ════════════════════════════════════════════════
-- Substitui a versão da 0019. Mesma assinatura, muda só `funil.leads_captados`.
--
-- Achado 2026-09-21: `agente_clientes` não tem coluna a dizer qual assistente
-- criou o registo, por isso `leads_captados`/`mqls` vinham idênticos em todos
-- os assistentes (contagem global da tabela, nunca filtrada por p_agente).
-- Decisão do utilizador: os assistentes passam a espelhar também em
-- `contactos` (coluna nova `agente`, ALTER TABLE à mão — ver
-- docs/fases/contactos-unificado-assistentes-plano.md, Opção B), e é de lá
-- que `leads_captados` passa a contar, agora sim filtrado por `p_agente`.
--
-- `mqls`/`taxa_qualificacao` CONTINUAM a vir de `agente_clientes` (`clientes`
-- CTE, inalterada) — `contactos` não tem orçamento/zona_preferida. Ficam por
-- isso ainda sem filtro por agente; limitação conhecida, registada no plano,
-- não resolvida nesta migration.
--
-- CREATE OR REPLACE FUNCTION apenas. Reversível reaplicando a 0019.
...
-- Fica sem filtro por agente (ver nota no topo do ficheiro): só alimenta MQL.
clientes as (
...
-- Espelho em `contactos`, com atribuição por assistente (coluna `agente`,
-- ALTER TABLE 2026-09-21). É daqui que `leads_captados` passa a contar.
contactos_captados as (
  select ct.* from contactos ct, janela j
  where ct.criado_em >= j.desde
    and ct.agente is not null
    and (p_agente is null or ct.agente = p_agente)
),
...
select jsonb_build_object(
  ...
  'funil', (
    select jsonb_build_object(
      'leads_captados', (select count(*) from contactos_captados),
...
comment on function agente_metricas(text, integer) is
  'Métricas por assistente em 4 blocos: funil, atendimento, preferencias, operacional. leads_captados vem de contactos.agente (0038).';
